...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--vcf', required=True, type=str,
                        metavar='<vcf.gz>', help='vcf.gz file')
...
def timecourse(vcf: str, output: str) -> None:
    ...
    for line in vcf:
        ...
        if allele_freq and sample_threshold:
            outline = ",".join([chr, str(pos), mutation, " ".join(str(t) for t in merged_times), " ".join(str(a) for a in allele_alts), " ".join(str(d) for d in allele_depths)])
            out.write(outline + "\n")
